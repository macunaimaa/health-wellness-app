...
export class RecommendationService {
  ...
  private engine(input: {
    ...
    const mealRecs = mealService.getRecommendations({
      contextType: checkin.context_type || 'home',
    ...
    if (shouldIncludeWorkouts) {
      const workoutRecs = workoutService.getRecommendations({
        contextType: checkin.context_type || 'home',
        availableMinutes: checkin.available_minutes || 30,
        energyLevel: checkin.energy_level,
    ...
    if (needsRecovery) {
      const recoveryRecs = recoveryService.getRecommendations({
        energyLevel: checkin.energy_level,
        stressLevel: checkin.stress_level,
        sleepQuality: checkin.sleep_quality,
        contextType: checkin.context_type || 'home',
    ...
    if (travelService.isTravelContext(checkin.context_type || '')) {
      const travelMeals = travelService.getTravelMeals(
        checkin.context_type || 'travel',
        profile.dietary_restrictions_json,
      );
      const travelTips = travelService.getTravelTips(checkin.context_type || 'travel');
      ...
      if (travelMeals.length > 0 && !recommendations.some(r => r.recommendation_type === 'meal' && r.title.includes('viagem'))) {
        ...
        if (!recentTitles.has(firstMeal.name)) {
          recommendations.push({
            ...
            payload_json: {
              where: firstMeal.where,
              isTravelSpecific: true,
            },
    ...
    recommendations.push({
      ...
      summary: `Beba pelo menos 2 litros de água hoje. Lembre-se de beber um copo a cada hora.`,
      rationale: checkin.energy_level <= 3
        ? 'Desidratação pode ser causa de fadiga. Manter hidratação adequada ajuda a recuperar energia.'
        : 'Manter hidratação consistente suporta performance física e mental ao longo do dia.',
      payload_json: {
        targetLiters: 2,
        intervalMinutes: 60,
        tips: [
          ...
          'Água com limão ou hortelã para variar',
